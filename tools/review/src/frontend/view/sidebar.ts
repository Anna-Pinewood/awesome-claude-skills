...
const buildSidebarView = (model: Model, dispatch: (msg: Msg) => void): VNode => {
...
// Summary is intentionally excluded: the textarea is a controlled-in-name-only
// input — user keystrokes drive model.summary, so the DOM already has the
// right value. `setSummary` is only ever dispatched from the textarea's input
// handler (verified via grep), so there's no code path that sets summary and
// expects the sidebar vnode to reflect it.
export const sidebarView = createMemo(
