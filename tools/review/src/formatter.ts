...
const resolveStatus = (submission: ReviewSubmission): "approved" | "changes_requested" =>
	submission.comments.length > 0 ? "changes_requested" : "approved";
...
export const formatReview = (
	...
	lines.push(`file_count: ${groupByFile(submission.comments).size}`);
	lines.push(`comment_count: ${submission.comments.length}`);
	lines.push("---");
	lines.push("");
