...
class TestGetVideoMetadata:
    """Test metadata fetching with mocked yt_dlp lib."""

    @patch("yt_subs._extract_info")
    ...
    @patch("yt_subs._get_cookies_opts", return_value={})
    @patch("yt_subs._extract_info", return_value=None)
    def test_failure_returns_none(self, mock_info, mock_cookies):
        title, channel = yt_subs.get_video_metadata("https://youtu.be/abc12345678")
        assert title is None
        assert channel is None

    @patch("yt_subs._extract_info")
    def test_only_title_no_channel(self, mock_info):
        mock_info.return_value = {"title": "Title Only"}
        title, channel = yt_subs.get_video_metadata("https://youtu.be/abc12345678")
...
class TestDownloadSubtitles:
    """Test subtitle downloading with mocked lib helpers."""

    @patch("yt_subs.get_video_metadata")
    @patch("yt_subs.try_download_subtitles")
    def test_success_cleaned(self, mock_try, mock_meta):
        mock_meta.return_value = ("Title", "Channel")
        vtt_content = "WEBVTT\n\n00:00:00.000 --> 00:00:02.000\nHello world"
        mock_try.return_value = ([("subs.en.vtt", vtt_content)], None)
    ...
    @patch("yt_subs.get_video_metadata")
    @patch("yt_subs.try_download_subtitles")
    def test_success_raw(self, mock_try, mock_meta):
        mock_meta.return_value = ("Title", "Channel")
        vtt_content = "WEBVTT\n\n00:00:00.000 --> 00:00:02.000\nHello world"
        mock_try.return_value = ([("subs.en.vtt", vtt_content)], None)
    ...
    @patch("yt_subs._get_cookies_opts", return_value={})
    @patch("yt_subs.list_available_subtitles", return_value=[])
    @patch("yt_subs.get_video_metadata")
    @patch("yt_subs.try_download_subtitles")
    def test_no_subtitles_found(
        self, mock_try, mock_meta, mock_list, mock_cookies, capsys
    ):
        mock_meta.return_value = ("Title", "Channel")
        ...
        assert title == "Title"
        captured = capsys.readouterr()
        assert "No subtitles available" in captured.err

    @patch("yt_subs._get_cookies_opts", return_value={})
    @patch("yt_subs.list_available_subtitles", return_value=[])
    @patch("yt_subs.get_video_metadata")
    @patch("yt_subs.try_download_subtitles")
    def test_yt_dlp_error(
        self, mock_try, mock_meta, mock_list, mock_cookies, capsys
    ):
        mock_meta.return_value = ("Title", "Channel")
        mock_try.return_value = ([], "Some fatal error")
        ...
        assert content is None
        captured = capsys.readouterr()
        assert "yt-dlp error" in captured.err
        assert "Some fatal error" in captured.err
